...
NODE_STYLES = {
    ...
    "ExternalIssue": {"color": "#3b82f6", "label": "External Issue"},
...
def _process_node(
    ...
    if node_id not in nodes_map:
        ...
        # If binary parsing failed or returned a raw ID (Label_XX), try to infer type from properties.
        # OR if it's "UserStory" (from dynamic parsing) but we want "Story" for UI consistency.
        if group == "UserStory":
            group = "Story"

        if group == "Unknown" or group.startswith("Label_") or group == "Document":
            # Even if it is "Document", check properties for stricter types that might have been lost in labels
            # or if we are dealing with a generic Document node that has properties implying a subtype

            # Re-run heuristic check if group is generic Document or Unknown
            pass
            # (Keep existing heuristic block effectively, but now 'group' might already be 'KnowledgeBase' so we skip it)

        if group == "Unknown" or group.startswith("Label_"):
            found_type = False

            # 1. Scan ALL property values for known display patterns
            # This is robust even if property *names* are lost (e.g. it's "prop_15" instead of "label_display")
            for val in props.values():
                if not isinstance(val, str):
                    continue

                # Check known prefixes generated by EntityOperations
                if val.startswith("Story:"):
                    group = "Story"
                    found_type = True
                    break
                elif val.startswith("Epic:"):
                    group = "Epic"
                    found_type = True
                    break
                elif val.startswith("Requirement:"):
                    group = "Requirement"
                    found_type = True
                    break
                elif val.startswith("Test ") or val.startswith("Test:"):
                    group = "TestCase"
                    found_type = True
                    break
                elif val.startswith("Risk:"):
                    group = "Risk"
                    found_type = True
                    break
                elif val.startswith("Ticket") or "Issue" in val:
                    group = "ExternalIssue"
                    found_type = True
                    break

            # 2. If signature not found in values, check structural properties (if keys are preserved)
            if not found_type:
                if (
                    ...
                    group = "Document"
                elif "email" in props and "role" in props:
                    group = "Stakeholder"
                elif "version" in props and "category" in props:
                    group = "Technology"
                elif "target_date" in props:
                    group = "Milestone"
