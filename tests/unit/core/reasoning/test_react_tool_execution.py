...
class TestToolTimeout:
    async def test_hung_async_tool_times_out(self) -> None:
        ...
        agent = _agent(
            ToolDefinition(name="hang", fn=hang, description="hangs", category="read_only"),
            tool_timeout=0.05,
        )
    ...
    async def test_no_timeout_by_default(self) -> None:
        ...
        agent = _agent(ToolDefinition(name="quick", fn=quick, description="fast", category="read_only"))
...
class TestToolRetry:
    async def test_transient_connection_error_retried(self) -> None:
        ...
        agent = _agent(
            ToolDefinition(name="flaky", fn=flaky, description="flaky", category="read_only"),
            tool_retries=1,
            retry_backoff=0.01,
        )
    ...
    async def test_non_transient_error_not_retried(self) -> None:
        ...
        agent = _agent(
            ToolDefinition(name="broken", fn=broken, description="broken", category="read_only"),
            tool_retries=3,
            retry_backoff=0.01,
        )
    ...
    async def test_timeout_not_retried(self) -> None:
        ...
        agent = _agent(
            ToolDefinition(name="slow", fn=slow, description="slow", category="read_only"),
            tool_timeout=0.05,
            tool_retries=3,
            retry_backoff=0.01,
        )
    ...
    async def test_retries_exhausted_returns_error(self) -> None:
        ...
        agent = _agent(
            ToolDefinition(name="down", fn=always_down, description="down", category="read_only"),
            tool_retries=2,
            retry_backoff=0.01,
        )
