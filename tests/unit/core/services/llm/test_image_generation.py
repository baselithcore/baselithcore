...
class _Provider:
    async def generate_image(self, prompt, *, model=None, size=None, **kwargs):
        self.seen = (prompt, model, size)
...
async def test_the_seam_returns_bytes_from_the_active_provider() -> None:
    ...
    image = await generate_image(service, "a lighthouse", model="m", size="1024x1024")

    assert image.data == PNG
    assert image.media_type == "image/png"
    assert service.provider.seen == ("a lighthouse", "m", "1024x1024")
