...
| `BASELITH_CACHE_XFETCH_BETA` | `1.0` (on) | Cache | XFetch probabilistic early refresh on `RedisTTLCache`: as an entry nears expiry, one caller probabilistically recomputes it *before* the TTL lapses while everyone else keeps being served — no synchronized cold key. `1.0` is the canonical setting; higher = earlier refreshes, lower = later; `0` disables. |
