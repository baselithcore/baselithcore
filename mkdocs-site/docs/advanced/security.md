...
| Dependency updates | **Dependabot** (`.github/dependabot.yml`), gated by CI | Weekly, **grouped** bump PRs (one per ecosystem: `uv` lock, GitHub Actions, the two npm trees) instead of one per package; majors stay individual PRs. Every bump runs the same gates as any PR — `uv lock --check`, `pip-audit` (on the base install **and** on the locked set with every non-conflicting extra, so torch/transformers/pypdf/playwright are covered), Trivy, the locked test matrix — so a CVE still surfaces as a red build, and the fix arrives as a reviewable PR. Version ceilings stay a reviewed decision — `anthropic` is capped `<1.0`, `openai` `<3.0` in `pyproject.toml`; base-image digests are refreshed by hand |
