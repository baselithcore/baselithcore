...
[CHANGELOG](https://keepachangelog.com) entry therefore travel in the
release-prep pull request, and the release job only tags and publishes.
...
1. **Announce** — mark the old surface deprecated in the same release that ships
   ...
   for HTTP/CLI surfaces). The release notes — which the CHANGELOG mirrors —
   are generated by semantic-release from commit subjects (`Bug Fixes` /
   `Features` / `Performance Improvements` sections, and there is no
   `Deprecated` section), so name the
